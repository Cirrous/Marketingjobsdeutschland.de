            ...
            <div class="flex flex-wrap justify-center">
                @foreach($tags as $tag)
                    <a href="{{ route('listings.index', ['tag' => $tag->slug]) }}"
                        class="inline-block w-1/6 p-2 tracking-wide text-xs font-black title-font border border-indigo-500 uppercase {{ $tag->slug === request()->get('tag') ? 'bg-indigo-500 text-white' : 'bg-white text-indigo-500' }} mr-4 mb-4 hover:bg-indigo-500 hover:text-white rounded-md text-center"
                    >{{ $tag->name }}</a>
                @endforeach
            </div>
        </div>
        <div class="mb-12">
            <h2 class="text-2xl font-medium text-gray-900 title-font px-4">Alle Jobs ({{ $listings->count() }})</h2>
        </div>
        <div class="-my-6">
